...
int main() {
	...
	std::cout << "\n--- ATTACK TEST ---" << std::endl;
	k.attack("Target Dummy");      // ScavTrap attack is used
	
	std::cout << "\n--- DAMAGE TEST ---" << std::endl;
	k.takeDamage(20);              // HP decreases
	k.takeDamage(90);              // HP should drop to 0
	k.attack("Another Target");    // Cannot attack, HP = 0 → should fail
	
	std::cout << "\n--- REPAIR TEST ---" << std::endl;
	k.beRepaired(10);              // Cannot repair if HP = 0
	
	std::cout << "\n--- WHO AM I TEST ---" << std::endl;
	k.whoAmI();                    		  // It prints the names of DiamondTrap and ClapTrap

	std::cout << "\n--- ENERGY DEPLETION TEST ---" << std::endl;
	DiamondTrap l("CHIAA");
	for (int i = 0; i < 51; ++i)
	    l.attack("Target");        // 50 attacks succeed, 51st fails → energy depleted
